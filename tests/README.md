...
| `tackle_zsh_smoke.sh` | Runs a create + `--done` cycle, a dependency-registry symlink case, **and** a project-config copy + `setup` hook under **zsh** (bats only covers bash) — guards the registry's array/`read`/`${//}` splitting and the config path's process-substitution / dynamic-scope / python-heredoc against zsh differences. |
| `run.sh` | Runs both. |
